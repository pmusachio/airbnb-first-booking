...
def _one_hot_encoder(sparse: bool):
...
def _preprocessor(X, dense: bool = False):
    """Build the numeric/categorical ColumnTransformer.

    Tree-based models such as HistGradientBoostingClassifier require dense
    input and are not sensitive to feature scaling, while linear models
    benefit from scaled, sparsity-friendly matrices.
    """
    ...
    categorical_pipe = Pipeline(
        steps=[
            ...
            ("onehot", _one_hot_encoder(sparse=not dense)),
        ]
    )
...
def _classifier(config: dict):
    ...
    model_type = modeling.get("model_type", "logistic_regression")
    ...
    if model_type == "gradient_boosting":
        ...
        return HistGradientBoostingClassifier(
            max_iter=int(modeling.get("n_estimators", 200)),
            learning_rate=float(modeling.get("learning_rate", 0.1)),
            max_depth=modeling.get("max_depth"),
...
def _requires_dense_input(config: dict) -> bool:
    return config.get("modeling", {}).get("model_type") in {"gradient_boosting", "random_forest"}


def _pipeline(X, estimator, config: dict):
    from sklearn.pipeline import Pipeline

    return Pipeline(steps=[("preprocess", _preprocessor(X, dense=_requires_dense_input(config))), ("model", estimator)])


def _ndcg_at_k(y_true, y_proba, classes, k: int = 5) -> float:
    """Mean NDCG@k for a multiclass ranking, as used by the Kaggle Airbnb challenge.

    Each row contributes 1 / log2(rank + 1) when the true class appears within
    the top-k scored classes, and 0 otherwise (ideal DCG is always 1, since
    there is a single relevant label per row).
    """
    import numpy as np

    classes = np.asarray(classes)
    y_true = np.asarray(y_true)
    k = min(int(k), len(classes))
    if k <= 0:
        return 0.0
    top_k = np.argsort(y_proba, axis=1)[:, ::-1][:, :k]
    scores = np.zeros(len(y_true))
    for rank in range(k):
        match = classes[top_k[:, rank]] == y_true
        scores[match] = 1.0 / np.log2(rank + 2)
    return float(scores.mean())


def _classification_metrics(y_true, y_pred, y_proba, config: dict, classes=None):
    import numpy as np
    ...
    if y_proba is not None and len(labels) > 2 and hasattr(y_proba, "shape") and classes is not None:
        for k in config.get("evaluation", {}).get("ndcg_k", [5]):
            metrics[f"ndcg_at_{int(k)}"] = _ndcg_at_k(y_true, y_proba, classes, k=int(k))
...
def train_supervised(config: dict):
    ...
    model = _pipeline(X_train, estimator, config)
    ...
    metrics = (
        ...
        else _classification_metrics(y_valid, y_pred, y_proba, config, classes=getattr(model, "classes_", None))
    )
...
def train_clustering(config: dict):
    ...
    preprocessor = _preprocessor(X, dense=_requires_dense_input(config))
